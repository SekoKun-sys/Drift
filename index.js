...
client.on('message', async message => {
   ...
   if(message.content.indexOf('https://bit-ly.ru/') != -1 || message.content.indexOf('bit-ly') != -1) return message.react('❌');
    ...
    .setTitle(`Найден повреждённый айди. Айди: ${ch}.`)
    .setThumbnail(`https://cdna.artstation.com/p/assets/images/images/007/919/772/large/maksim-batyrev-untitled-233333333.jpg?1509364469`)
    .setTimestamp()
    
   if(ch == message.channel.id) return message.react(`✉`);
   let chU = client.channels.cache.get(ch)

   if(!chU) return client.channels.cache.get("702059843875373076").send(emb)
...
client.on("guildCreate", guild => {
    ...
    .setTitle(`Я зашёл на сервер!`)
    .addField(`Имя сервера`, guild.name)
    .addField(`ID сервера`, guild.id)
    .addField(`ID владельца`, guild.ownerID)
    .addField("Тег владельца", guild.ownerTag)
    .addField(`Кол-во серверов`, client.guilds.cache.size)
    .setThumbnail(`https://cdna.artstation.com/p/assets/images/images/007/919/772/large/maksim-batyrev-untitled-233333333.jpg?1509364469`)
    .setColor("GREEN")
    client.channels.cache.get("702059843875373076").send(e)
  })
  
  client.on("guildDelete", guild => {
    let e = new Discord.MessageEmbed()
    .setTitle(`Я вышел с сервера`)
    .addField(`Имя сервера`, guild.name)
    .addField(`ID сервера`, guild.id)
    .addField(`ID владельца`, guild.ownerID)
    .addField("Тег владельца", guild.ownerTag)
